 ...
 * All variables/methods are declared as static (i.e., class) in this non-OO version
 */

public class TTTConsoleNonOO {
    // Defined named constants for:
    ...
    public static void initGame() {
        ...
        currentPlayer = PLAYING; // ready to play
    }

    /**  
     * The currentPlayermakes one move (one step)
     * Update board[selectedRow][selectedCol] and currentState
     */
     ...
     * Update board[selectedRow][selectedCol]
     * Compute and return the new game state (PLAYING, DRAW, CROSS_WON, NOUGHT_WON)
            ...
            for (int row = 0; row < ROWS; ++row) {
                for (int col = 0; col < COLS; ++col) {
                    if (board[row][col] == NO_SEED) {
                        return PLAYING; // still have empty celss
                    }
                }
            }
